...
8. **`type=news` — это КОНТЕНТ, не промо, и пишется иначе:**
   ...
   - **Проверка вида спорта:** если новость явно НЕ про футбол (соккер) — например крикет, регби, теннис, баскетбол — поставь `compliance.ok=false`, в `notes` укажи «не футбол», и НЕ пиши пост. Эмодзи в заголовке должно соответствовать футболу.
